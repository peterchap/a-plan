...
for file in files:
    ...
    df = remove_bad_statuss(df)
    c = df['data flag'].value_counts()
    stats = stats.append(pd.DataFrame.from_dict([[product,'Bad statuss', str(c[2] - b[2])]]))
    ...
    stats.to_csv(directory_2 + product +  "removestats_all_"+ month + ".csv", index=True)           
    ...
    for i, x in df.groupby(['CODEDT']):
        ...
        for j, m in x.groupby('data flag'):
            print("File",i,j, m.shape[0])
            print(list(m.columns.values))
            ...
            print(ispsplit.shape)
            #ispsplit.loc['Product'] = product
            #ispsplit.loc['List'] = i
            #ispsplit.loc['Segment'] = j
            ispsplit.insert(0,'Segment',j,True)
            ispsplit.insert(0,'List',i,True)
            ispsplit.insert(0,'Product',product,True)
            
            ispstats = ispstats.append(ispsplit)
            to_mdropcols = ['data flag', 'CODEDT']
            m.drop(to_mdropcols, axis=1, inplace=True)
            #m.to_csv(directory_2 + status + "_" + i + "_"+ j + month + ".csv", index=False)
...
ispstats.to_csv(directory_2 +  "ipstats_all_"+ month + ".csv", index=True)
#filecounts.to_csv(directory_2 + "filecounts_all_" + month + ".csv", index=False)