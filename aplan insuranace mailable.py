...
directory_2 = 'E:/A-Plan June 2020/A-Plan June Final Data/'
...
files = ['3955_A_Plan_Jun_Car_insurance_Branch_ESB_C.csv',\
'3955_A_Plan_Jun_Car_insurance_National_ESB_C.csv',\
'3955_A_Plan_Jun_Home_insurance_ESB_C.csv']
...
dfs = df2.drop_duplicates(subset='email', keep='first')
df2 = dfs[dfs['primary_membership'].notnull() & dfs['user_status']  == 'Mailable' ]
...
for file in files:
    ...
    df3 = pd.merge(df1, df2, left_on='Email', right_on='email', how='inner')
 
    print(file, df3.shape)
"""
    if "Branch" in file:
        product = "Car"
    ...
    for i, x in df.groupby(['CODEDT']):
        ...
        for j, m in x.groupby('data flag'):
            ...
            msft.to_csv(directory_2 + product + "_" + i + "_"+ j + month + "_microsoft.csv", index=False)
            other.to_csv(directory_2 + product + "_" + i + "_"+ j + month + "_non_microsoft.csv", index=False)
