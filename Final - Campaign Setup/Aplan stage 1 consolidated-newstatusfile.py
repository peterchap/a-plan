...
for file in files:
    a = pd.read_csv(
        directory + file, encoding="ISO-8859-1", low_memory=False, usecols=email
    )
    print(a.shape)
    stats = stats.append(
        pd.Series([file, a.shape[0]], index=stats.columns), ignore_index=True
    )
    df = pd.concat([df, a])
print(stats)
...
df.loc[
    ((df["is_banned_word"] == 1) & (df["status"] == "OK")), "status"
] = "Banned words"
...
df3 = pd.read_csv(
    onedrive2 + "TLDGeneric lookup.csv", encoding="ISO-8859-1", low_memory=False
)
...
print(df.columns)
df.to_csv(
    directory + "A_Plan_" + month + "_newstatus_Stage1Complete_v2.csv", index=False
)
